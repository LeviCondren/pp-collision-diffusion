...
Trains on dijet, ttbar, wjets, zjets from full_event_mixed/ using the same
stage-1 architecture as E023 (bsm_grid_event_c_stage1): stage-1 ResNet predicts
8-dim combined target [log_npart, log1p_MET, sin_MET_phi, cos_MET_phi,
log1p_cone_pT_X, log1p_cone_mass_X, log1p_cone_pT_Y, log1p_cone_mass_Y].

Key differences from bsm_grid_train_event_c_stage1.py (E023):
  - Loads 4 SM process files instead of 144 W' signal mass-point files.
  - No mass conditioning (mass_x=mass_y=0 always).
  - Event features always computed from truth particle data (no zeroing).
  - No holdout point exclusion.
  - Stats path defaults to ckpt_dir (not sm_dir) to prevent contamination.
"""
...
NUM_FEAT       = 6
NUM_JET        = 8   # E028/E023: [log_npart, log1p_MET, sin_MET_phi, cos_MET_phi,
                     #              log1p_cone_pT_X, log1p_cone_mass_X,
                     #              log1p_cone_pT_Y, log1p_cone_mass_Y]
MAX_PARTONS    = 4
PARTON_FEAT    = 7
NUM_EVENT_FEAT = 7   # truth 7-dim event features for stage-2 head
R_CONE         = 1.0

# sin/cos indices in the combined 8-dim jet vector: sin_MET=2, cos_MET=3
...
def _sm_files(sm_dir):
    return [os.path.join(sm_dir, f'{p}.hdf5') for p in SM_PROCESSES]


# ── Combined 8-dim stats (SM 4-process) ──────────────────────────────────────

def compute_combined_stats(sm_dir, num_part, val_start):
    """Compute 8-dim combined stats from SM training data.

    jet_mean/std are 8-element: [log_npart_stat, ev_feat[0..6]_stat].
    sin/cos components (indices 2, 3) are fixed at mean=0, std=1.
    """
    files    = _sm_files(sm_dir)
    ...
    for fpath in files:
        ...
        combined = np.concatenate([log_n[:, None], raw7], axis=1)  # (N, 8)
...
def load_stats(stats_path):
    ...
    if len(jet_mean) != NUM_JET:
        raise ValueError(
            f"Stats file has {len(jet_mean)}-dim jet_mean, expected {NUM_JET} (E028).")
    return stats
...
def load_sm_shard(sm_dir, stats, hvd_rank, hvd_size,
    ...
    jet_mean_s = float(jet_mean_a[0])   # log_npart mean
    ...
    for fpath, r0, r1 in file_slices:
        ...
        jet_s0    = (log_npart - jet_mean_s) / jet_std_s   # (N, 1)
        pf6_norm  = (pf6 - part_mean) / part_std * mask[:, :, None]

        raw7    = _compute_event_raw_all7(pf_raw, part_raw, num_part)
        ev_norm = (raw7 - ev_mean) / ev_std                # (N, 7)

        # E028: combined 8-dim jet target = [log_npart, event_feat[0..6]]
        ...
        del parton_mask, mask, jet_s0, ew, jet_combined, raw7, ev_norm
...
def parse_args():
    ...
    p.add_argument('--run_name',         default='sm_4proc_event_c_stage1')
    p.add_argument('--stats_path',       default=None,
                   help='Path to 8-dim combined stats JSON '
                        '(default: {ckpt_dir}/{run_name}/normalisation_stats_event_c_stage1_sm4proc.json)')
    p.add_argument('--num_jet_mlp',      type=int,   default=512,
                   help='ResNet width for stage-1 (default 512, matching E023)')
...
def main():
    ...
    sm_dir   = flags.sm_dir
    ckpt_dir = flags.ckpt_dir or os.path.join(sm_dir, 'checkpoints_sm_4proc')
    run_dir  = os.path.join(ckpt_dir, flags.run_name)

    # Stats live in run_dir to avoid contaminating sm_dir with smoke-test stats.
    stats_path = (flags.stats_path
                  or os.path.join(run_dir,
                                  'normalisation_stats_event_c_stage1_sm4proc.json'))
    ...
    if hvd.rank() == 0 and not os.path.exists(stats_path):
        print("Computing combined 8-dim stats from SM data ...", flush=True)
