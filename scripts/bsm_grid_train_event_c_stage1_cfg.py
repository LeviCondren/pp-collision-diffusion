"""BSM grid training — E034: CFG training on cone mass conditioning.

Copied from bsm_grid_train_event_c_stage1.py (E023) and modified for E034:
  - Architecture identical to E023 (stage-1 ResNet, 8-dim combined target).
  - Training change: conditioning dropout on cone mass features of input_event.
    With probability cfg_dropout_prob (default 0.15), indices 4 and 6 of the
    7-dim event vector (log1p_cone_mass_X, log1p_cone_mass_Y) are set to 0.0
    (normalized zero = training mean). Other features unchanged.
  - Tracks 'cfg_drop' metric: should be ~cfg_dropout_prob each epoch.
  - Stats file: normalisation_stats_event_c_stage1_cfg.json (same format as E023).
  - Run name: bsm_grid_event_c_stage1_cfg

Do NOT modify the original bsm_grid_train_event_c_stage1.py (E023 canonical).

Changelog:
  E034 (2026-08-01): CFG conditioning dropout on cone mass features.
"""
...
from PET_pp_parton_vpar_bsm_event_c_stage1_cfg import WeightedBSMPET_event_c_cfg
...
def parse_args():
    ...
    p.add_argument('--cfg_dropout_prob',  type=float, default=0.15,
                   help='Fraction of training events with cone mass conditioning dropped (default 0.15)')
...
def main():
    ...
    if hvd.rank() == 0 and not os.path.exists(stats_path):
        print("Computing combined E034 stats ...", flush=True)
    ...
    if hvd.rank() == 0:
        print(f"num_jet={NUM_JET}  num_event_feat={NUM_EVENT_FEAT}  run={flags.run_name}  "
              f"cfg_dropout_prob={flags.cfg_dropout_prob}", flush=True)
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = WeightedBSMPET_event_c_cfg(
        num_feat=NUM_FEAT,
        num_jet=NUM_JET,
        ...
        cfg_dropout_prob=flags.cfg_dropout_prob,
    )
