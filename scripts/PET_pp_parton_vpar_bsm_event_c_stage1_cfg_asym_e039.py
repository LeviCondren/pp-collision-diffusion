"""PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym_e039 — E039: joint mass-hypothesis
+ cone-mass CFG dropout.

Copied from PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym.py (E035/E036/E038) and
modified for E039, ported from the sandbox implementation
(pp-collision-diffusion_sandbox/scripts/Pet_pp_e039.py, smoke-tested there 2026-09-15):
  - train_step now ALSO zeros mass_x/mass_y in the parton-conditioning vector y
    whenever cone_mass_X/Y is dropped — using the SAME drop_X/drop_Y draws, so
    cone_mass_X and mass_x are always dropped together (never one without the
    other), and likewise for Y. Closes the leakage path identified in
    EXPERIMENTS.md's E039 entry: previously mass_x/mass_y (fixed, in y) was
    NEVER dropped even when cone_mass_X/Y was, so the model could partly infer
    the "missing" cone mass from the ever-present mass hypothesis, weakening
    the v_full/v_null contrast CFG guidance depends on.
  - Two new module-level 32-dim keep-mask constants, _MASS_X_KEEP_32 /
    _MASS_Y_KEEP_32 (zero only the injected mass_col feature at flat cond
    index 20 / 27 respectively — parton slot 2/3, feature 6). Two new
    per-instance constant tensors built from them in __init__.
  - Everything else (model_jet, model_part, samplers, EMA, loss) is
    byte-identical to PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym.py.
  - Known obstacles NOT addressed by this change (see EXPERIMENTS.md E039
    entry for full discussion):
      1. Harder null distribution to learn — not mitigated here beyond
         reusing E038's already-raised 0.35/0.35 dropout rate.
      2. Per-axis (X-only/Y-only) guidance for stage-1 itself is still not
         architecturally supported (model_jet's _resnet_vpar mean-pools all
         4 partons) — this change does not touch stage-1 sampling/guidance,
         only train-time dropout.
      3. Leakage not fully closed: mass_col only overwrites 1 of 7 features
         on parton slots 2/3; the other 6 genuine kinematic columns of those
         same partons are still never dropped — this implementation zeros
         only the injected mass feature, deliberately mirroring how cone-mass
         dropout only zeros its own single feature.
      4. y/cond construction is still a static NumPy array built once at
         data-load time; this change only adds a dropout-mask multiply on
         the already-built y tensor inside train_step, not a rebuild.

Original E035 docstring follows, for the architecture this file inherits from:
...
Changelog:
  ...
  E039 (2026-09-17): ported from sandbox — joint mass_x/mass_y + cone_mass_X/Y
    CFG dropout (same drop_X/drop_Y draws drive both).
"""
...
# E039: analogous keep-masks for the 32-dim parton-conditioning vector y/cond
# ( = 28-dim cond_norm [4 partons x 7 features, flattened] + 4-dim parton_mask ).
# mass_x/mass_y are injected via mass_col into parton slot 2 / slot 3's 7th
# feature (see train.py's mass_col[:, 2, 0] / mass_col[:, 3, 0]), which land
# at flat cond_norm index (slot*7 + 6): slot 2 -> 2*7+6=20, slot 3 -> 3*7+6=27.
_MASS_X_KEEP_32 = [0. if i == 20 else 1. for i in range(32)]  # zero only index 20 (mass_x)
_MASS_Y_KEEP_32 = [0. if i == 27 else 1. for i in range(32)]  # zero only index 27 (mass_y)
...
class WeightedBSMPET_event_c_cfg_asym(WeightedBSMPET_event_c):
    ...
    def __init__(self, *args, cfg_drop_x_prob=0.10, cfg_drop_y_prob=0.10, **kwargs):
        ...
        # E039: matching keep masks for y/cond's mass_x (idx 20) / mass_y (idx 27)
        self._mass_x_keep_mask  = tf.constant(_MASS_X_KEEP_32, dtype=tf.float32)  # zeros index 20 (mass_x)
        self._mass_y_keep_mask  = tf.constant(_MASS_Y_KEEP_32, dtype=tf.float32)  # zeros index 27 (mass_y)
    ...
    def train_step(self, inputs):
        ...
        # E039: apply the SAME drop_X/drop_Y draws to y/cond's mass_x (idx 20) /
        # mass_y (idx 27), so cone_mass_X and mass_x are always dropped together
        # (never one without the other), and likewise for Y. Closes the leakage
        # path where mass_x/mass_y was never dropped even when cone_mass_X/Y was.
        y_after_x = tf.where(drop_X[:, None],
                              y * self._mass_x_keep_mask[None, :],
                              y)
        y_input = tf.where(drop_Y[:, None],
                            y_after_x * self._mass_y_keep_mask[None, :],
                            y_after_x)
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            v_pred_part = self.model_part([
                perturbed_x * mask,
                perturbed_x[:, :, :2] * mask,
                x['input_mask'], x['input_jet'][:, 0:1], t, y_input,
                event_input])   # ← per-cone dropout-applied event + parton conditioning (E039)
            v_pred_part = tf.reshape(v_pred_part, (batch_size, -1))
            v_part      = alpha[:, None] * eps - sigma[:, None] * x['input_features']
            v_part      = tf.reshape(v_part, (batch_size, -1))
