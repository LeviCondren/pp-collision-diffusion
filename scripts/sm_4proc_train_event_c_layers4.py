"""E027 SM 4-process training with E022 architecture.

Trains on dijet, ttbar, wjets, zjets from full_event_mixed/ using the same
event-level conditioning (MET + cone_X + cone_Y) and architecture as E022
(bsm_grid_event_c_layers4).

Key differences from bsm_grid_train_event_c_layers4.py:
  - Loads 4 explicit SM process files instead of 144 W' signal mass-point files.
  - Always computes event features (no mass-point holdout or background zeroing).
  - val_start=480000: train on [0:480k], validate on [480k:490k], holdout [490k:500k].
  - No mass conditioning (mass_col is always zeros).
"""
...
def compute_stats(sm_dir, num_part, val_start):
    ...
    for fpath in files:
        ...
        N = len(part_raw)
        mass_col = np.zeros((N, MAX_PARTONS, 1), dtype=np.float64)  # always zero for SM
...
def load_sm_shard(sm_dir, stats, event_stats, hvd_rank, hvd_size,
    ...
    for fpath, r0, r1 in file_slices:
        ...
        # Parton conditioning — mass column always zeros for SM processes
        ...
        # Event features — always computed for SM (no mass conditioning zeroing)
