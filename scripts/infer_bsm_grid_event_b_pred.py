#!/usr/bin/env python3
"""Phase 2 BSM grid inference — E020b: event-level cone_X conditioning, predicted features.
...
Two-pass generation:
  ...
  Pass 2: Run E020b (PET_pp_parton_vpar_bsm_event_b) conditioned on those
...
E020b features: log1p(cone_pT_X), log1p(cone_mass_X) — NUM_EVENT_FEAT = 2
"""
...
MASS_NORM      = 600.0
NUM_EVENT_FEAT = 2   # E020b: log1p(cone_pT_X), log1p(cone_mass_X)
R_CONE         = 1.0
...
def _assemble_event_feat(raw7):
    """Variant b: cone_X only (2 features: log1p_cone_pT_X, log1p_cone_mass_X)."""
    return raw7[:, 3:5]
...
event_raw    = _assemble_event_feat(raw7)                            # (N, 2)
...
print(f'\n[rank {args.rank}] === PASS 2: E020b ({args.num_steps} steps) ===')
...
print(f'[rank {args.rank}] Loaded E020b: {ckpt_path}')
