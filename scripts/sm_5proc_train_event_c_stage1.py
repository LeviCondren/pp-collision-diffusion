"""SM 5-process training — E030: stage-1 diffusion predicts 8-dim event vector.

Same architecture as E023 (bsm_grid_event_c_stage1 / PET_pp_parton_vpar_bsm_event_c_stage1)
but trained on the 5 SM process files (dijet, ttbar, wjets, zjets, wprime) from
full_event_mixed/ instead of the W' BSM mass grid.

Key differences from bsm_grid_train_event_c_stage1.py (E023):
  - Data: 5 fixed SM HDF5 files; val_start=480000 (train [0:480k], val [480k:490k]).
  - No mass conditioning: mass column always zero for all events.
  - No holdout points, no background/signal distinction; all events get event features.
  - All 5 processes contribute to jet (combined 8-dim) target.

Architecture (unchanged from E023):
  - model_jet: partons → 8-dim DDPM [log_npart, log1p_MET, sin_MET, cos_MET,
                                      log1p_coneX_pT, log1p_coneX_mass,
                                      log1p_coneY_pT, log1p_coneY_mass]
  - model_part: partons + log_npart + 7 truth event features → particles
"""
...
MAX_PARTONS     = 4
PARTON_FEAT     = 7   # 6 kinematic + 1 mass (always 0 for SM)
NUM_EVENT_FEAT  = 7
R_CONE          = 1.0

_SINCOS_INDICES_COMBINED = [2, 3]  # sin/cos MET phi in the 8-dim combined vector

_SM_PROCESSES = ['dijet', 'ttbar', 'wjets', 'zjets', 'wprime']


def _sm_files(sm_dir):
    return [os.path.join(sm_dir, f'{p}.hdf5') for p in _SM_PROCESSES]


# ── Event feature computation (identical to E023) ────────────────────────────
...
def compute_combined_stats(sm_dir, num_part, val_start):
    """8-dim combined stats from all 5 SM process files."""
    all_files = _sm_files(sm_dir)
    ...
    for fpath in all_files:
        ...
        raw7     = _compute_event_raw_all7(pf_f32, prt_f32, num_part).astype(np.float64)
        combined = np.concatenate([log_n[:, None], raw7], axis=1)
...
def load_stats(stats_path):
    ...
    if len(jet_mean) != NUM_JET:
        raise ValueError(
            f"Stats '{stats_path}' has {len(jet_mean)}-dim jet_mean, expected {NUM_JET}.")
    cond_mean = np.array(stats['cond_mean'], dtype=np.float32)
    expected_cond = MAX_PARTONS * PARTON_FEAT
    if len(cond_mean) != expected_cond:
        raise ValueError(
            f"Stats '{stats_path}' has {len(cond_mean)} cond dims, expected {expected_cond}.")
    return stats


# ── Data loading ──────────────────────────────────────────────────────────────

def load_sm_shard(sm_dir, stats, hvd_rank, hvd_size,
                  val_start, n_events, num_part, split):
    ...
    for fpath in _sm_files(sm_dir):
        with h5py.File(fpath, 'r') as f:
            ...
            if r0 >= r1: continue
        file_slices.append((fpath, r0, r1))

    if not file_slices:
        raise RuntimeError(f"No events loaded for split='{split}' at rank {hvd_rank}.")

    total_n  = sum(r1 - r0 for _, r0, r1 in file_slices)
    ...
    offset = 0
    for fpath, r0, r1 in file_slices:
        with h5py.File(fpath, 'r') as f:
        ...
        # Parton conditioning — mass column always zero for SM
        ...
        # All SM events get event features (no background zeroing)
        raw7    = _compute_event_raw_all7(pf_raw, part_raw, num_part)
        ev_norm = (raw7 - ev_mean) / ev_std

        # 8-dim combined jet target
        ...
        del parton_mask, mask, jet_s0, ew, raw7, ev_norm, jet_combined
...
def main():
    ...
    sm_dir     = flags.sm_dir
    ckpt_dir   = flags.ckpt_dir or os.path.join(sm_dir, 'checkpoints_sm_5proc')
    stats_path = (flags.stats_path
                  or os.path.join(ckpt_dir, 'normalisation_stats_sm5proc_stage1.json'))
    ...
    # ── Compute stats (rank 0) ────────────────────────────────────────────────
    if hvd.rank() == 0 and not os.path.exists(stats_path):
        print("Computing SM 5-proc stage-1 stats ...", flush=True)
        stats_dict = compute_combined_stats(sm_dir, flags.num_part, flags.val_start)
    ...
    (tr_pf, tr_mask, tr_cond, tr_jet, tr_w, tr_ev,
     part_mean, part_std) = load_sm_shard(
        sm_dir=sm_dir, stats=stats,
