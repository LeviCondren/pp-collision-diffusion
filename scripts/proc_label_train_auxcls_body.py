...
class ProcLabelPET(PET_pp_parton_vpar):
    ...
    def __init__(self, num_proc_labels, cfg_drop_prob=0.0, aux_weight=0.1, **kwargs):
        self._num_proc_labels = num_proc_labels
        self.cfg_drop_prob    = cfg_drop_prob
        self.aux_weight       = aux_weight
        super().__init__(**kwargs)
        # Build the auxiliary classifier on the body output (E007 design).
        # Must be called after super().__init__() has built self.body.
        self.aux_classifier   = self._build_aux_classifier()
        self.loss_aux_tracker = keras.metrics.Mean(name="aux")

    # ── Auxiliary classifier on body output ───────────────────────────────────

    def _build_aux_classifier(self):
        """Masked mean-pool over body output → predict process label.

        Gradients flow back through the body, enforcing process-discriminative
        representations in the encoder. Unlike E006, the classifier input is a
        learned representation shaped by the diffusion task, not a direct Dense
        projection of the one-hot label — so the task is non-trivial."""
        D           = self.projection_dim
        aux_body_in = Input(shape=(None, D), name='auxcls_body_in')
        aux_mask_in = Input(shape=(None, 1), name='auxcls_mask_in')

        mask_sum = tf.reduce_sum(aux_mask_in, axis=1)                        # (N, 1)
        pooled   = (tf.reduce_sum(aux_body_in * aux_mask_in, axis=1)
                    / (mask_sum + 1e-10))                                     # (N, D)
        x      = layers.Dense(D, activation='gelu', name='auxcls_dense1')(pooled)
        logits = layers.Dense(self._num_proc_labels, name='auxcls_classifier')(x)
        return keras.Model(inputs=[aux_body_in, aux_mask_in],
                           outputs=logits, name='aux_classifier')

    @property
    def metrics(self):
        return [self.loss_tracker, self.loss_part_tracker,
                self.loss_jet_tracker, self.loss_aux_tracker]
    ...
    def _build_vpar_generator_head(self):
        ...
        # Named layers so _build_cls_model can retrieve and share them.
...
class WeightedProcLabelPET(ProcLabelPET):
    """ProcLabelPET with per-batch |event_weight|-weighted DDPM loss plus
    an auxiliary cross-entropy classification loss on the proc_token embedding."""
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            eps         = tf.random.normal(tf.shape(x['input_features']),
                                           dtype=tf.float32) * mask
            perturbed_part = alpha[:, None] * x['input_features'] + eps * sigma[:, None]

            v_pred_part = self.model_part([
                perturbed_part * mask,
                perturbed_part[:, :, :2] * mask,
                x['input_mask'], x['input_jet'], t, y])
            v_pred_part = tf.reshape(v_pred_part, (batch_size, -1))
            v_part      = alpha[:, None] * eps - sigma[:, None] * x['input_features']
            v_part      = tf.reshape(v_part, (batch_size, -1))
            ...
            # Auxiliary classification loss on the body output (E007 design).
            # Run body explicitly so aux_classifier can consume its output.
            # model_part will re-run body internally (~5% extra cost, acceptable).
            body_out = self.body(
                [perturbed_part * mask, perturbed_part[:, :, :2] * mask,
                 x['input_mask'], t],
                training=True)                                  # (N, num_part, D)

            P, PF           = self.max_partons, self.parton_feat
            proc_label_true = y[:, P * PF + P:]                # (N, n_proc)
            aux_logits      = self.aux_classifier([body_out, mask], training=True)
            loss_aux        = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(
                    labels=proc_label_true, logits=aux_logits))

            loss = loss_jet + loss_part + self.aux_weight * loss_aux
            loss_body   = loss_part + self.aux_weight * loss_aux

        # Body gets gradients from both diffusion loss and aux classification loss.
        self.body_optimizer.minimize(
            loss_body,
            self.body.trainable_variables, tape=tape)
        trainable_vars = (self.model_jet.trainable_variables
                          + self.head.trainable_variables
                          + self.aux_classifier.trainable_variables)
        self.optimizer.minimize(loss, trainable_vars, tape=tape)

        self.loss_tracker.update_state(loss)
        self.loss_part_tracker.update_state(loss_part)
        self.loss_jet_tracker.update_state(loss_jet)
        self.loss_aux_tracker.update_state(loss_aux)
...
def parse_args():
    ...
    p.add_argument('--aux_weight',       type=float, default=0.1,
                   help='Weight of the auxiliary proc-token classification loss.')
    return p.parse_args()
