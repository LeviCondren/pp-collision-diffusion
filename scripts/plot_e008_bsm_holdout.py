#!/usr/bin/env python3
"""
Inference comparison plots for the E008 BSM-grid holdout results.
Treats the 4 held-out mass points as separate "processes" and produces
the same 5 plot types as plot_infer_wprime_holdout.py.

Format differences vs. old wprimeGrid holdout NPZs:
  - filenames: bsm_mX*.npz  (not mX*.npz)
  - mass_x / mass_y are scalar float attrs  (not 1-element arrays)
  - parton_feat is (N, 4, 7)  — 7th feature is mass/600, rest same as before

Usage:
    module load tensorflow/2.15.0   # provides pyjet
    python3 plot_e008_bsm_holdout.py \\
        --infer_dir /pscratch/sd/l/lcondren/MCsim/wprime_signal/checkpoints_bsm_grid/bsm_grid/infer_holdout_ep019_5k \\
        --out_dir   /pscratch/sd/l/lcondren/MCsim/wprime_signal/checkpoints_bsm_grid/bsm_grid/plots_ep019
"""
...
OUT_DIR = args.out_dir or os.path.join(
    os.path.dirname(args.infer_dir), 'plots_ep019')
os.makedirs(OUT_DIR, exist_ok=True)
N = args.n_events

# ── Load all mass points ──────────────────────────────────────────────────────
npz_files = sorted(glob.glob(os.path.join(args.infer_dir, 'bsm_mX*.npz')))
assert npz_files, f'No bsm_mX*.npz files found in {args.infer_dir}'

mass_points = {}
for path in npz_files:
    d = np.load(path)
    # mass_x / mass_y are scalar floats in E008 NPZs (not 1-element arrays)
    mX = float(d['mass_x'])
    mY = float(d['mass_y'])
    key = f'mX{int(mX):04d}_mY{int(mY):04d}'
    label = rf"$m_{{X}}$={int(mX)} GeV, $m_{{Y}}$={int(mY)} GeV"
    # parton_feat is (N,4,7): features [log_E,sinφ,cosφ,pz/p,pdg_norm,charge,mass/600]
    # η for outgoing partons derived from pz/p via arctanh; 7th feature ignored
    mass_points[key] = {
...
fig.suptitle('Particle distributions — E008 BSM holdout  (Truth vs Generated)', fontsize=11)
...
if n_pts == 1: axes = axes[np.newaxis, :]
fig.suptitle("Global event observables — E008 BSM holdout  (Truth vs Generated)", fontsize=11)
...
def _cluster_and_measure(parts_arr, mask_arr, parton_feat_arr):
    ...
    for i in range(N_):
        if i % 1000 == 0: print(f'    clustering {i}/{N_} ...', flush=True)
