"""E027 SM 4-process training — single-GPU, no Horovod (HPC3 version).

Identical logic to sm_4proc_train_event_c_layers4.py but runs on one GPU
without Horovod. Intended for clusters where only tensorflow is available.

Usage:
  CUDA_VISIBLE_DEVICES=0 python3 sm_4proc_train_singlegpu.py [args]
"""

# ── Minimal Horovod stub ──────────────────────────────────────────────────────
# All hvd.rank()==0 guards become unconditional; hvd.size()==1 so LR/batch
# scaling is a no-op; allreduce/broadcast return their input unchanged.
class _HvdStub:
    def init(self): pass
    def rank(self): return 0
    def local_rank(self): return 0
    def size(self): return 1
    def allreduce(self, t, name=''): return t
    def broadcast(self, t, root_rank=0): return t
    def DistributedOptimizer(self, opt, **kw): return opt
    class callbacks:
        class BroadcastGlobalVariablesCallback:
            def __init__(self, root_rank=0): pass
...
# These silently fail on HPC3 — harmless.
for _lib in [
...
if gpus:
    ...
    print(f"Using GPU: {gpus[0].name}", flush=True)
else:
...
from PET_pp_parton_vpar_bsm_event_c_layers4 import WeightedBSMPET_event_c
from tensorflow.keras.callbacks import ModelCheckpoint, ReduceLROnPlateau, EarlyStopping
from tensorflow.keras.optimizers import schedules, Adam
from PET import FourierProjection
from layers import LayerScale, StochasticDepth

_SM_DIR_DEFAULT = '/pub/lcondren/MCsim/full_event_mixed'
...
def compute_stats(sm_dir, num_part, val_start):
...
def compute_event_stats(sm_dir, num_part, val_start):
    files = _sm_files(sm_dir)
    ...
    for fpath in files:
        ...
        feat_sum += raw7.sum(0).astype(np.float64)
        feat_sq  += (raw7 ** 2).sum(0).astype(np.float64)
        N_total  += len(raw7)
...
def load_sm_shard(sm_dir, stats, event_stats, val_start, n_events, num_part, split):
    part_mean = np.array(stats['part_mean'], dtype=np.float32)
    ...
    for fpath in all_files:
        with h5py.File(fpath, 'r') as f:
            ...
            if n_events is not None:
                e = min(s + n_events, e)
            if s >= e: continue
        file_slices.append((fpath, s, e))

    if not file_slices:
        raise RuntimeError(f"No events loaded for split='{split}'.")

    total_n  = sum(e - s for _, s, e in file_slices)
    ...
    for fpath, s, e in file_slices:
        ...
        ev_norm = _normalize_event(raw7, event_stats)
        ...
        del parton_mask, mask, jet, ew, raw7, ev_norm

    rng = np.random.default_rng(42 if split == 'train' else 0)
    idx = rng.permutation(total_n)

    print(f"  [{split}] loaded {total_n:,} events", flush=True)
...
def parse_args():
    ...
    p.add_argument('--val_start',          type=int, default=20000)
...
def main():
    ...
    stats_path       = flags.stats_path or os.path.join(sm_dir, 'normalisation_stats_sm4proc.json')
    ...
    if not os.path.exists(stats_path):
        print("Computing parton conditioning stats from SM data ...", flush=True)
        stats_dict = compute_stats(sm_dir, flags.num_part, flags.val_start)
        with open(stats_path, 'w') as f:
            json.dump(stats_dict, f, indent=2)
        print(f"Stats saved to {stats_path}", flush=True)

    if not os.path.exists(stats_event_path):
        print("Computing event feature stats from SM data ...", flush=True)
        ev_stats_dict = compute_event_stats(sm_dir, flags.num_part, flags.val_start)
    ...
    lr_head          = flags.lr
    lr_body          = flags.lr_body

    print("Loading training data ...", flush=True)
    (tr_pf, tr_mask, tr_cond, tr_jet, tr_w, tr_ev,
     part_mean, part_std, jet_mean, jet_std) = load_sm_shard(
        sm_dir=sm_dir, stats=stats, event_stats=event_stats,
    ...
    print(f"Batch: {per_worker_batch}  |  Train: {n_local_train:,}  |  "
    ...
    print(f"Training: {n_local_train:,} events | "
