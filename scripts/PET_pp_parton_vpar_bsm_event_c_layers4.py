"""PET_pp_parton_vpar_bsm_event_c — E020c: event-level full conditioning (MET + cone_X + cone_Y).
...
Copied from PET_pp_parton_vpar_bsm.py (Phase 2 canonical) and modified for E020a:
  ...
  - Event features: [log(MET+1), sin(MET_phi), cos(MET_phi), log(cone_pT_X+1), log(cone_mass_X+1), log(cone_pT_Y+1), log(cone_mass_Y+1)]
...
class PET_pp_parton_vpar_bsm_event_c(keras.Model):
    def __init__(self,
                 num_feat,
                 num_jet,
                 max_partons=4,
...
class WeightedBSMPET_event_c(PET_pp_parton_vpar_bsm_event_c):
    """|event_weight|-weighted training loss for E020c."""
