#!/usr/bin/env python3
"""
Generate all comparison plots from the 100K inference npz files.
Reproduces every plot in per_parton_cond_infer.ipynb without re-running inference.

Usage:
    python plot_infer_100k.py [--run_name parton_v1_1node] [--n_events 100000]
"""
...
p.add_argument('--n_events',  type=int, default=100000)
p.add_argument('--data_dir',  type=str,
               default='/pscratch/sd/l/lcondren/MCsim/full_event_fpcd')
p.add_argument('--infer_dir', type=str, default=None,
               help='Directory containing *_100k.npz files (default: {ckpt}/{run_name}/infer_100k)')
p.add_argument('--out_dir',   type=str, default=None)
args = p.parse_args()

CKPT_BASE = f'{args.data_dir}/checkpoints_pet_pp'
NPZ_DIR   = args.infer_dir or f'{CKPT_BASE}/{args.run_name}/infer_100k'
OUT_DIR   = args.out_dir or f'{CKPT_BASE}/{args.run_name}/plots_100k'
os.makedirs(OUT_DIR, exist_ok=True)

N = args.n_events
NPART = 500
MODEL_LABEL = f'PET_pp {args.run_name}  ({N//1000}K events)'
...
fig.suptitle(f'PET_pp — Particle distributions  [{MODEL_LABEL}]', fontsize=11)

for row, proc in enumerate(['dijet','zjets']):
...
print('Plot 3: jet observables (clustering 100K events — takes ~15 min) ...')
...
def _cluster_and_measure(parts_arr,mask_arr,parton_feat_arr):
    ...
    for i in range(N_):
        if i%10000==0: print(f'    clustering {i}/{N_} ...',flush=True)
...
for _proc in ['dijet','zjets']:
    ...
    fig,axes=plt.subplots(7,_n_cols,figsize=(3.5*_n_cols,26),constrained_layout=True)
...
for _proc in ['dijet','zjets']:
    ...
    fig,axes=plt.subplots(len(_active_fl),2,figsize=(9,4.2*len(_active_fl)),constrained_layout=True)
    axes=np.atleast_2d(axes)
...
for _proc in ['dijet','zjets']:
    ...
    _n_cols=len(_plot_fl)
    fig,axes=plt.subplots(len(_PCOBS),_n_cols,figsize=(3.5*_n_cols,4*len(_PCOBS)),constrained_layout=True)
    axes=np.atleast_2d(axes)
