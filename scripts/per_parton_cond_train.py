"""Train PET_pp_parton with per-parton cross-attention conditioning.

Identical to train_pp_horovod.py except:
  - Uses PET_pp_parton (per-parton cross-attention head + parton-pooled ResNet)
  - Default run_name = 'parton_v1_hvd'
  - Exposes --num_gen_layers (generator head depth, default 2)

See PET_pp_parton.py for the architecture description.

Launch (multi-node via srun):
  srun python3 per_parton_cond_train.py [args]

Launch (single-node smoke, 1 GPU via horovodrun):
  horovodrun --gloo -np 1 python3 per_parton_cond_train.py \
      --run_name parton_smoke --batch 16 --epoch 1 \
...
def parse_args():
    ...
    p.add_argument('--run_name',          default='parton_v1_hvd')
...
def load_shard(data_dir, stats_path, processes, hvd_rank, hvd_size,
    ...
    for proc in processes:
        ...
        cond_raw  = part[:, :NUM_PARTONS, :].reshape(part.shape[0], NUM_COND)
