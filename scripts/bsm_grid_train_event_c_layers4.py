"""Phase 2 BSM grid training — E020c: event-level full conditioning (MET + cone_X + cone_Y).
...
Copied from bsm_grid_train.py and extended for E020a:
  ...
  - Normalization stats for event features stored in normalisation_stats_event_c.json.
...
def parse_args():
    ...
    p.add_argument('--run_name',           default='bsm_grid_event_c_layers4')
    p.add_argument('--stats_path',         default=None)
    p.add_argument('--stats_event_path',   default=None,
                   help='Path to normalisation_stats_event_c.json '
                        '(default: {grid_dir}/normalisation_stats_event_c.json)')
...
def main():
    ...
    if hvd.rank() == 0 and not os.path.exists(stats_event_path):
        print(f"Computing event feature stats (E020c) ...", flush=True)
