"""Phase 2 BSM grid training — E020b: event-level cone_X conditioning.
...
Copied from bsm_grid_train.py and extended for E020a:
  ...
  - Normalization stats for event features stored in normalisation_stats_event_b.json.
...
from PET_pp_parton_vpar_bsm_event_b import WeightedBSMPET_event_b
...
_SINCOS_INDICES = []   # no sin/cos features in variant b
...
def _assemble_event_feat(raw7):  # variant b: cone_X only
    """Extract variant-a features (MET only) from full 7-feature array."""
    return raw7[:, 3:5]  # log(cone_pT_X+1), log(cone_mass_X+1)
...
def parse_args():
    ...
    p.add_argument('--stats_event_path',   default=None,
                   help='Path to normalisation_stats_event_b.json '
                        '(default: {grid_dir}/normalisation_stats_event_b.json)')
...
def main():
    ...
    if hvd.rank() == 0 and not os.path.exists(stats_event_path):
        print(f"Computing event feature stats (E020b) ...", flush=True)
