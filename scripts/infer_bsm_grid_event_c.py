#!/usr/bin/env python3
"""Phase 2 BSM grid inference — E020c: event-level full conditioning.

Copied from infer_bsm_grid.py and extended for E020c:
  - Reads truth event features (MET) from holdout HDF5 data.
  - Normalizes using normalisation_stats_event_c.json.
...
def _parse():
    ...
    p.add_argument('--stats_event_path', default=None,
                   help='Event stats JSON (default: {grid_dir}/normalisation_stats_event_c.json)')
...
if not os.path.exists(stats_event_path):
    raise FileNotFoundError(
        ...
        f"Run bsm_grid_train_event_c.py first to compute them.")
