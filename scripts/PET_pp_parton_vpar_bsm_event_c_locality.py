"""PET_pp_parton_vpar_bsm_event_c_locality — E033: per-parton mass conditioning with locality bias.

Copied from PET_pp_parton_vpar_bsm_event_c_stage1.py (E023/E032) and modified:

  Stage-1 (model_jet) is identical: predicts 8-dim event vector from parton conditioning.

  Stage-2 generator head (_build_vpar_generator_head) replaces the global event token
  with two per-parton mass tokens — one for the X decay (parton slot 2) and one for the
  Y decay (parton slot 3).  Each mass token is built from the parton's direction features
  (normalised sinφ, cosφ, β_z, and the full 7-dim parton feature vector) concatenated
  with the predicted (or truth) cone mass for that decay.

  A locality gate controls how strongly each particle attends to each mass token.
  The gate is a learned sigmoid function of the current particle representation
  (after layer-norm) and the relative position of the particle with respect to the
  parton's direction in normalised (η, sinφ) space.  This gives stage-2 an explicit
  inductive bias: particles near parton slot 2 should be more strongly influenced by
  cone_mass_X, and similarly for slot 3 and cone_mass_Y.

  The cross-attention in each generator layer is split into three independent streams:
    (a) standard MHA to 4 parton tokens      — no locality bias (same as E032)
    (b) MHA to mass-X token × gate_X         — locality-gated
    (c) MHA to mass-Y token × gate_Y         — locality-gated
  Outputs are summed before the residual connection.

  To expose particle positions to the head, inputs_points (the noised [η, sinφ] of all
  particles) is now routed from model_part through to the head.  The external call
  signature of model_part is unchanged — inputs_points was already the 2nd input.

  train_step / test_step: identical to WeightedBSMPET_event_c (parent API unchanged).
  generate() / DDPMSampler: evaluate_models passes x[:,:,:2] to the head at every step.

Do NOT modify the original PET_pp_parton_vpar_bsm_event_c_stage1.py (E032 canonical).
Do NOT mix checkpoints from this file with E032 checkpoints.
"""
...
_NUM_EVENT_FEAT_DEFAULT = 7  # MET(3) + cone_X(2) + cone_Y(2)

# Event feature indices (inp_event layout):
#  0: log1p(MET)       1: sin(MET_phi)    2: cos(MET_phi)
#  3: log1p(cone_pT_X) 4: log1p(cone_mass_X)
#  5: log1p(cone_pT_Y) 6: log1p(cone_mass_Y)
_CONE_MASS_X_IDX = 4
_CONE_MASS_Y_IDX = 6

# Parton feature layout (per-slot in inp_cond, 7 dims):
#  0: feat0  1: sinφ  2: cosφ  3: β_z (pz/E ≈ tanh η)  4: pdg_norm  5: mask  6: mass/600
_PARTON_SINPHI_IDX = 1
_PARTON_BETAZ_IDX  = 3


class PET_pp_parton_vpar_bsm_event_c_locality(keras.Model):
    def __init__(self,
                 num_feat,
                 num_jet,
                 max_partons=4,
        ...
        # ── Body encoder (identical to E032) ──────────────────────────────────
        ...
        # ── Locality generator head ───────────────────────────────────────────
        self.head = self._build_locality_generator_head()

        # ── Wire body + head into model_part ──────────────────────────────────
        # inputs_points is the 2nd input (noised [η, sinφ] for all particles).
        # Previously routed only to body; now also forwarded to head for locality gates.
        inputs_time     = Input((1,))
        ...
        outputs_head = self.head([output_body, inputs_jet_s2, inputs_mask, inputs_time,
                                  inputs_cond, inputs_event, inputs_points])
        ...
        # ── Stage-1 ResNet: 8-dim [log_npart + event features] (unchanged) ───
    ...
    def _build_locality_generator_head(self):
        ...
        inp_points  = Input(shape=(None, 2),         name='vph_points')  # noised [η, sinφ]

        # ── Parse parton tokens ───────────────────────────────────────────────
        parton_feat_flat = inp_cond[:, :P * PF]               # (N, P*PF)
        parton_mask_in   = inp_cond[:, P * PF : P * PF + P]   # (N, P)

        parton_tokens = layers.Reshape((P, PF))(parton_feat_flat)   # (N, P, 7)
        parton_emb    = layers.Dense(D)(parton_tokens)               # (N, P, D)
        parton_emb    = StochasticDepth(self.feature_drop)(parton_emb)

        # ── Per-parton mass tokens ────────────────────────────────────────────
        # cone_mass_X / cone_mass_Y from the event feature vector
        cone_mass_X = inp_event[:, _CONE_MASS_X_IDX:_CONE_MASS_X_IDX + 1]  # (N, 1)
        cone_mass_Y = inp_event[:, _CONE_MASS_Y_IDX:_CONE_MASS_Y_IDX + 1]  # (N, 1)

        # Concatenate parton direction features (all 7) with the cone mass target
        mass_inp_X = tf.concat([parton_tokens[:, 2, :], cone_mass_X], axis=-1)  # (N, PF+1)
        mass_inp_Y = tf.concat([parton_tokens[:, 3, :], cone_mass_Y], axis=-1)  # (N, PF+1)

        mass_token_X = layers.Dense(D, activation='gelu', name='mass_tok_X_1')(mass_inp_X)
        mass_token_X = layers.Dense(D, name='mass_tok_X_2')(mass_token_X)
        mass_token_X = tf.expand_dims(mass_token_X, axis=1)                     # (N, 1, D)

        mass_token_Y = layers.Dense(D, activation='gelu', name='mass_tok_Y_1')(mass_inp_Y)
        mass_token_Y = layers.Dense(D, name='mass_tok_Y_2')(mass_token_Y)
        mass_token_Y = tf.expand_dims(mass_token_Y, axis=1)                     # (N, 1, D)

        # ── Relative position features for locality gates ─────────────────────
        # inp_points carries noised [η, sinφ] for every particle; these are in the
        # same normalised feature space as the parton tokens, so differences are
        # directly comparable without denormalisation.
        eta_part  = inp_points[:, :, 0:1]   # (N, P_max, 1) — normalised η (particle)
        sphi_part = inp_points[:, :, 1:2]   # (N, P_max, 1) — normalised sinφ (particle)

        # Parton direction proxies from normalised parton tokens:
        #   β_z = pz/E ≈ tanh(η_parton) — monotone in η, good proximity proxy
        #   sinφ — monotone in φ for |φ| < π/2
        # Shape: (N, 1, 1) — broadcast over particle dim
        eta_proxy_X  = parton_tokens[:, 2:3, _PARTON_BETAZ_IDX  :_PARTON_BETAZ_IDX  + 1]
        sphi_proxy_X = parton_tokens[:, 2:3, _PARTON_SINPHI_IDX :_PARTON_SINPHI_IDX + 1]
        eta_proxy_Y  = parton_tokens[:, 3:4, _PARTON_BETAZ_IDX  :_PARTON_BETAZ_IDX  + 1]
        sphi_proxy_Y = parton_tokens[:, 3:4, _PARTON_SINPHI_IDX :_PARTON_SINPHI_IDX + 1]

        # Relative positions (N, P_max, 2) — broadcast subtracts parton direction
        rel_X = tf.concat([eta_part - eta_proxy_X, sphi_part - sphi_proxy_X], axis=-1)
        rel_Y = tf.concat([eta_part - eta_proxy_Y, sphi_part - sphi_proxy_Y], axis=-1)

        # ── Parton attention mask (4 partons only — no event token) ───────────
        attn_mask = tf.cast(parton_mask_in[:, None, :], tf.bool)  # (N, 1, P)

        # ── Time + jet conditioning token (unchanged) ─────────────────────────
        ...
        for i in range(self.num_gen_layers):
            ...
            # 2a. Standard cross-attention to parton tokens (no locality needed;
            #     partons condition the full particle cloud uniformly)
            parton_cross = layers.MultiHeadAttention(
                num_heads=nh, key_dim=kd, name=f'parton_xattn_{i}')(
                query=x2n, key=parton_emb, value=parton_emb,
                attention_mask=attn_mask)
            parton_cross = parton_cross * inp_mask

            # 2b. Cross-attention to mass-X token, scaled by locality gate.
            # Gate is a learned sigmoid function of the current particle
            # representation and its relative position to parton X.
            # Shape: mass_X_cross (N, P, D), gate_X (N, P, 1)
            mass_X_cross = layers.MultiHeadAttention(
                num_heads=nh, key_dim=kd, name=f'mass_X_xattn_{i}')(
                query=x2n, key=mass_token_X, value=mass_token_X)
            gate_X = tf.sigmoid(layers.Dense(1, name=f'gate_X_{i}')(
                tf.concat([x2n, rel_X], axis=-1)))
            mass_X_cross = mass_X_cross * gate_X * inp_mask

            # 2c. Cross-attention to mass-Y token, scaled by locality gate.
            mass_Y_cross = layers.MultiHeadAttention(
                num_heads=nh, key_dim=kd, name=f'mass_Y_xattn_{i}')(
                query=x2n, key=mass_token_Y, value=mass_token_Y)
            gate_Y = tf.sigmoid(layers.Dense(1, name=f'gate_Y_{i}')(
                tf.concat([x2n, rel_Y], axis=-1)))
            mass_Y_cross = mass_Y_cross * gate_Y * inp_mask

            # Sum all cross-attention contributions
            cross = parton_cross + mass_X_cross + mass_Y_cross
            if self.layer_scale:
        ...
        return keras.Model(
            inputs=[inp_encoded, inp_jet, inp_mask, inp_time,
                    inp_cond, inp_event, inp_points],
            outputs=out,
            name='vpar_generator_head_locality')
    ...
    def generate(self, cond, jet_mean, jet_std, event_feat,
        ...
        API identical to PET_pp_parton_vpar_bsm_event_c_stage1.generate().
        Locality gates use the noised particle positions at each DDPM step
        (extracted inside evaluate_models via x[:,:,:2]).
        """
        ...
        for i, split in (tqdm(enumerate(splits), total=len(splits))
            ...
            ev_i = ev_splits[i] if use_true_event else jet[:, 1:]
    ...
    def evaluate_models(self, head, body, x, jet, mask, t, cond, w=0.0,
                        event_feat=None):
        # x[:,:,:2] = noised [η, sinφ] at this diffusion step.
        # Passed to both body (unchanged) and head (for locality gates).
        x_in   = mask * x
        points = x[:, :, :2]
        v_body = body([x_in, points, mask, t], training=False)
        v      = mask * head([v_body, jet, mask, t, cond, event_feat, points],
                              training=False)
        return v
...
class WeightedBSMPET_event_c_locality(PET_pp_parton_vpar_bsm_event_c_locality):
    """|event_weight|-weighted training loss for E033."""
