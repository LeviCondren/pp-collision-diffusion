#!/usr/bin/env python3
"""
Inference script for the proc-label 5-process model (proc_label_train.py).
...
Extends infer_pp_5proc.py to use ProcLabelPET (MAX_PARTONS=6, num_cond=47).
...
def _parse():
    ...
    p.add_argument('--guidance_scale', type=float, default=0.0,
                   help='CFG guidance scale (0=no guidance, >0 amplifies process conditioning)')
    return p.parse_args()
