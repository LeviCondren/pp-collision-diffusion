"""BSM grid training — E033: per-parton mass conditioning with locality bias.

Copied from bsm_grid_train_event_c_stage1.py (E023/E032) and modified for E033:
  - Imports WeightedBSMPET_event_c_locality from PET_pp_parton_vpar_bsm_event_c_locality.
  - Default run_name: bsm_grid_event_c_locality.
  - Stats file: normalisation_stats_event_c_stage1.json (same 8-dim format; data unchanged).
  - All hyperparameters, data loading, and training loop are identical to E032.

Architecture change (in PET_pp_parton_vpar_bsm_event_c_locality.py):
  Stage-2 generator replaces the global event token with two per-parton mass tokens
  (cone_mass_X, cone_mass_Y), each gated by the particle's proximity to the corresponding
  parton direction in (η, sinφ) space.  Stage-1 ResNet is unchanged.

Do NOT modify bsm_grid_train_event_c_stage1.py (E032 canonical).
"""
...
from PET_pp_parton_vpar_bsm_event_c_locality import WeightedBSMPET_event_c_locality
...
def load_stats(stats_path):
    ...
    if len(jet_mean) != NUM_JET:
        raise ValueError(
            f"Stats file has {len(jet_mean)}-dim jet_mean, expected {NUM_JET} (E033).")
    return stats
...
def main():
    ...
    # ── Compute combined 8-dim stats if not present (rank 0) ─────────────────
    if hvd.rank() == 0 and not os.path.exists(stats_path):
        print("Computing combined E033 stats ...", flush=True)
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = WeightedBSMPET_event_c_locality(
        num_feat=NUM_FEAT,
        num_jet=NUM_JET,
