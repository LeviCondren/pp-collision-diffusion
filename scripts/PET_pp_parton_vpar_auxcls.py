...
class PET_pp_parton_vpar(keras.Model):
    ...
    def generate(self, cond, jet_mean, jet_std, nsplit=2, jets=None, use_tqdm=False,
                 num_steps=None, guidance_scale=0.0):
        ...
        for i, split in (tqdm(enumerate(splits), total=len(splits))
            ...
            parts = self.DDPMSampler(
                ...
                guidance_scale=guidance_scale).numpy()
    ...
    def evaluate_models(self, head, body, x, jet, mask, t, cond, w=0.0,
        ...
        v_cond = mask * head([v_body, jet, mask, t, cond], training=False)

        if guidance_scale > 0:
            P, PF = self.max_partons, self.parton_feat
            cond_uncond = tf.concat([
                cond[:, :P*PF + P],
                tf.zeros_like(cond[:, P*PF + P:]),
            ], axis=1)
            v_uncond = mask * head([v_body, jet, mask, t, cond_uncond], training=False)
