#!/usr/bin/env python3
"""BSM grid inference — E035: four-pass per-cone asymmetric CFG for cone mass conditioning.

Copied from infer_bsm_grid_event_c_stage1_cfg.py (E034) and modified for E035:
  - Loads E035 asym stats from normalisation_stats_event_c_stage1_cfg_asym.json.
    Pass --stats_path to use E034 stats if asym stats don't exist yet.
  - Default run_name: bsm_grid_event_c_stage1_cfg_asym.
  - Four-pass per-cone CFG inference:
      v_full:  all event features
  ...
  - --guidance_scale_X and --guidance_scale_Y replace the single --guidance_scale.
  - Uses model.generate_asym_cfg() from PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym.

Do NOT modify the original infer_bsm_grid_event_c_stage1_cfg.py (E034 canonical).

Changelog:
  E034 (2026-08-01): two-pass symmetric CFG inference.
  E035 (2026-08-14): four-pass per-cone asymmetric CFG inference.
"""
...
def _parse():
    ...
    p.add_argument('--guidance_scale_X',  type=float, default=3.0,
                   help='Per-cone CFG guidance scale for cone_mass_X. '
                        '0=no X guidance, 1=unit X guidance, >1=extrapolation. (default 3.0)')
    p.add_argument('--guidance_scale_Y',  type=float, default=3.0,
                   help='Per-cone CFG guidance scale for cone_mass_Y. '
                        '0=no Y guidance, 1=unit Y guidance, >1=extrapolation. (default 3.0)')
...
if args.stage1_only:
    ...
    print(f'[rank {args.rank}] asym CFG  guidance_scale_X={args.guidance_scale_X}  '
          f'guidance_scale_Y={args.guidance_scale_Y}  '
    ...
    if args.use_true_event:
        ...
        # Four-pass per-cone asymmetric CFG generation
        parts_gen, jets_gen = model.generate_asym_cfg(
            cond=cond,
            jet_mean=jet_mean,
            jet_std=jet_std,
            nsplit=nsplit,
