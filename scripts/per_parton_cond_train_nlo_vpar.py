"""NLO training with variable-length parton conditioning.

Extends per_parton_cond_train_nlo.py to handle variable numbers of hard-scatter
partons per event (4 for 2→2 processes, 5 for 2→3 NLO real-emission processes).

Key differences from per_parton_cond_train_nlo.py:
  - MAX_PARTONS = 5  (was 4)
  - NUM_COND    = MAX_PARTONS * PARTON_FEAT + MAX_PARTONS = 35
      The extra MAX_PARTONS dims are the binary parton mask appended to cond.
  - load_shard builds a parton_mask from the n_partons HDF5 field (preferred)
      or falls back to zero-norm detection for old 4-parton datasets.
  - cond_mean/cond_std are extended to MAX_PARTONS*PARTON_FEAT dims if the stats
      file was computed on fewer partons.
  - Uses PET_pp_parton_vpar (masked cross-attention + masked mean-pool).

Data requirement:
  HDF5 datasets must have parton_features shape (N, 5, 6) for full vpar support.
  Use generate_full_events_nlo.py (updated MAX_PARTONS=5) to regenerate data.
  The script also works with old (N, 4, 6) data by zero-padding to 5 partons.

Launch (multi-node via srun):
  srun python3 per_parton_cond_train_nlo_vpar.py [args]

Launch (single-node smoke, 1 GPU via horovodrun):
  horovodrun --gloo -np 1 python3 per_parton_cond_train_nlo_vpar.py \
      --run_name parton_nlo_vpar_smoke --batch 16 --epoch 1 \
...
NUM_JET     = 1    # log_npart
MAX_PARTONS = 5    # 2 initial + up to 3 final (covers 2→3 NLO real emission)
PARTON_FEAT = 6
# cond = [normalized parton kinematics (MAX_PARTONS*PARTON_FEAT)] + [parton mask (MAX_PARTONS)]
NUM_COND    = MAX_PARTONS * PARTON_FEAT + MAX_PARTONS  # 35


class WeightedPET_pp_parton_vpar(PET_pp_parton_vpar):
    """PET_pp_parton_vpar with per-batch |event_weight|-weighted DDPM loss.

    Positive-weight events only (negative-weight events are dropped in load_shard).
    Loss uses per-event per-particle MSE weighted by normalised event weight.
    """
...
def parse_args():
    ...
    p.add_argument('--run_name',         default='parton_v1_nlo_vpar')
...
def load_shard(data_dir, stats_path, processes, hvd_rank, hvd_size,
    ...
    # Extend cond_mean/cond_std to max_partons*PARTON_FEAT dims if the stats file
    # was computed on a dataset with fewer partons (e.g. old 4-parton data).
    ...
    for proc in processes:
        ...
        with h5py.File(path, 'r') as f:
            ...
            end_global = min(end_global, n_total)  # clamp to file size
            if n_events_per_proc is not None:
                end_global = min(start_global + n_events_per_proc, end_global)

            n_proc   = max(end_global - start_global, 0)
            ...
            part_raw = f['parton_features'][r0:r1].astype(np.float32)  # (N, P_file, 6)
            ew       = f['event_weights'][r0:r1].astype(np.float32)

            # Build parton mask from n_partons field if present; else from zero-norm
            if 'n_partons' in f:
                n_par = f['n_partons'][r0:r1].astype(np.int32)  # (N,)
                parton_mask = (np.arange(max_partons)[None, :] <
                               n_par[:, None]).astype(np.float32)  # (N, max_partons)
            else:
                p_norms = np.linalg.norm(part_raw, axis=2)   # (N, P_file)
                valid   = (p_norms > 1e-6).astype(np.float32)
                if valid.shape[1] < max_partons:
                    pad = np.zeros((len(valid), max_partons - valid.shape[1]), dtype=np.float32)
                    valid = np.concatenate([valid, pad], axis=1)
                parton_mask = valid[:, :max_partons]          # (N, max_partons)

        # Zero-pad parton features to max_partons if file has fewer
        P_file = part_raw.shape[1]
        if P_file < max_partons:
            pad = np.zeros((len(part_raw), max_partons - P_file, PARTON_FEAT), dtype=np.float32)
            part_raw = np.concatenate([part_raw, pad], axis=1)
        part_raw = part_raw[:, :max_partons, :]                # (N, max_partons, 6)
        ...
        # Normalise parton kinematics; padded slots remain zero (mean=0, std=1)
        cond_raw  = part_raw.reshape(len(part_raw), max_partons * PARTON_FEAT)
        cond_norm = (cond_raw - cond_mean) / cond_std          # (N, max_partons*PARTON_FEAT)

        # Append parton mask to form extended conditioning vector
        cond = np.concatenate([cond_norm, parton_mask], axis=1)  # (N, NUM_COND)
...
def main():
    ...
    num_cond = flags.max_partons * PARTON_FEAT + flags.max_partons
    ...
    tr_pf, tr_mask, tr_cond, tr_jet, tr_w, part_mean, part_std, jet_mean, jet_std = load_shard(
        ...
        max_partons=flags.max_partons, split='train')
    ...
    vl_pf, vl_mask, vl_cond, vl_jet, vl_w, _, _, _, _ = load_shard(
        ...
        max_partons=flags.max_partons, split='val')

    steps_per_epoch = n_local_train // per_worker_batch

    if hvd.rank() == 0:
        print(f"max_partons={flags.max_partons}  num_cond={num_cond}", flush=True)
    ...
    model = WeightedPET_pp_parton_vpar(
        num_feat=NUM_FEAT,
        num_jet=NUM_JET,
    ...
    if hvd.rank() == 0:
        hist_path = os.path.join(flags.data_dir, 'checkpoints', 'histories', f'{flags.run_name}.pkl')
