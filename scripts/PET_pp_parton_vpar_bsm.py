"""PET_pp_parton_vpar_bsm — Phase 2 BSM-parameterized generator.

Copied from PET_pp_parton_vpar.py (Phase 1 canonical) and modified for Phase 2:
  - max_partons reduced 6 → 4 (matches wprime_signal data: 2 incoming + X + Y)
  - parton_feat extended 6 → 7 (added mass as 7th feature)
  - Process labels removed entirely (no num_proc_labels, no proc_token)
  - Conditioning vector: 4*7 + 4 = 32 dims total

Conditioning vector layout (32-dim):
  [0:28]   normalised parton kinematics  (4 partons × 7 features)
           features: log_E, sin_phi, cos_phi, pz/E, pdg_norm, occupancy, mass_norm
  [28:32]  binary parton mask  (1 = valid; always [1,1,1,1] in wprime_signal data)

Parton slot convention (from generate_wprime_signal.py):
  slot 0: incoming quark, beam-A  → mass_norm = 0
  slot 1: incoming quark, beam-B  → mass_norm = 0
  slot 2: X (W boson, m=m_X)     → mass_norm = m_X / 600
  slot 3: Y (Z boson, m=m_Y)     → mass_norm = m_Y / 600
  Background events: m_X = m_Y = 0 → all-zero mass

Model class hierarchy (Phase 2):
  PET_pp_parton_vpar_bsm     base (unweighted train_step, for reference)
  └─ WeightedBSMPET          used for training (|w|-weighted loss)

Do NOT modify this file. It is the Phase 2 canonical architecture.
Do NOT mix with Phase 1 checkpoints (different num_cond, parton_feat, max_partons).
"""
...
class PET_pp_parton_vpar_bsm(keras.Model):
    def __init__(self,
                 num_feat,
                 num_jet,
                 max_partons=4,
    ...
    # ── Generator head: masked per-parton cross-attention (no process label) ──
    ...
    def _build_vpar_generator_head(self):
        ...
        parton_feat_flat = inp_cond[:, :P * PF]               # (N, P*PF = 28)
        parton_mask_in   = inp_cond[:, P * PF : P * PF + P]   # (N, P = 4)
        ...
        # No process token: key/value set is just parton embeddings
        cond_set  = parton_emb                                       # (N, 4, D)
        attn_mask = tf.cast(parton_mask_in[:, None, :], tf.bool)     # (N, 1, 4)
    ...
    def _resnet_vpar(self, inputs, inputs_time, labels,
        ...
        parton_feat_flat = labels[:, :P * PF]               # (N, 28)
        parton_mask_in   = labels[:, P * PF : P * PF + P]   # (N, 4)

        parton_tokens = tf.reshape(parton_feat_flat, (-1, P, PF))   # (N, 4, 7)
        parton_emb    = layers.Dense(D)(parton_tokens)               # (N, 4, D)

        mask_expand   = parton_mask_in[:, :, None]                   # (N, 4, 1)
        count         = tf.maximum(
    ...
    def generate(self, cond, jet_mean, jet_std, nsplit=2, jets=None, use_tqdm=False,
                 num_steps=None):
        """Generate particle clouds. cond shape: (N, 32)."""
...
class WeightedBSMPET(PET_pp_parton_vpar_bsm):
    """|event_weight|-weighted training loss for the BSM grid.

    Signal/background distinguished by mass features alone; all 4 parton slots
    are always occupied so the parton mask is [1,1,1,1] for every event.
    """
