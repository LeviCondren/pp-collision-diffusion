...
def fix_met(parts_phys, mask_gen, jets_gen, ev_mean, ev_std):
    """Post-process generated particle cloud to match stage-1 predicted MET.

    Stage-1 predicts [log1p(MET), sin(MET_phi), cos(MET_phi)] in normalized
    space (jets_gen[:, 1:4]).  After stage-2 particle generation the particle
    cloud's pT vector sum typically does not match that prediction.  This
    function applies a uniform (delta_px, delta_py) to all valid particles so
    that the cloud's sum_px/sum_py exactly matches the stage-1 target.

    MET convention follows _compute_event_raw_all7: MET vector = sum(pT*cos_phi),
    sum(pT*sin_phi) — NOT the negative sum.

    Args:
        parts_phys : (N, npart, 6) float32 — denormalised generated particles
                     features: [eta, sin_phi, cos_phi, log_pT, pdg_norm, charge]
        mask_gen   : (N, npart) float32 — 1 for valid particles, 0 for padding
        jets_gen   : (N, 8)  float32 — stage-1 output in normalised space
        ev_mean    : (7,) float32 — event feature means  (ev_mean[0..2] = MET slots)
        ev_std     : (7,) float32 — event feature stds

    Returns:
        parts_out  : (N, npart, 6) float32 — corrected particles (copy of input)
    """
    parts_out = parts_phys.copy()
    mask = mask_gen.astype(bool)                                   # (N, npart)

    # ── Decode stage-1 predicted MET ────────────────────────────────────────
    log1p_met_pred = jets_gen[:, 1] * float(ev_std[0]) + float(ev_mean[0])  # (N,)
    met_mag_pred   = np.expm1(np.clip(log1p_met_pred, -10, 20))              # (N,) GeV
    sp_pred = jets_gen[:, 2] * float(ev_std[1]) + float(ev_mean[1])          # sin
    cp_pred = jets_gen[:, 3] * float(ev_std[2]) + float(ev_mean[2])          # cos
    # Re-normalise sin/cos to a unit vector (model output may not be exact)
    rn = np.sqrt(sp_pred**2 + cp_pred**2 + 1e-8)
    sp_pred = sp_pred / rn;  cp_pred = cp_pred / rn

    met_x_tgt = met_mag_pred * cp_pred                                        # (N,)
    met_y_tgt = met_mag_pred * sp_pred                                        # (N,)

    # ── Current particle-level MET ───────────────────────────────────────────
    pT = np.exp(np.clip(parts_phys[:, :, 3], -10, 10)) * mask                # (N, npart)
    met_x_cur = (pT * parts_phys[:, :, 2]).sum(1)                            # sum(pT*cos) (N,)
    met_y_cur = (pT * parts_phys[:, :, 1]).sum(1)                            # sum(pT*sin) (N,)

    # ── Uniform correction across valid particles ────────────────────────────
    n_valid = mask.sum(1).clip(min=1).astype(float)                           # (N,)
    d_px = ((met_x_tgt - met_x_cur) / n_valid)[:, None] * mask               # (N, npart)
    d_py = ((met_y_tgt - met_y_cur) / n_valid)[:, None] * mask               # (N, npart)

    px_new = pT * parts_phys[:, :, 2] + d_px                                 # (N, npart)
    py_new = pT * parts_phys[:, :, 1] + d_py                                 # (N, npart)
    pT_new = np.sqrt(px_new**2 + py_new**2)

    # Only update particles whose corrected pT is physically sensible (> 10 MeV)
