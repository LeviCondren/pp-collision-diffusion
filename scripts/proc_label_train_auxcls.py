...
class ProcLabelPET(PET_pp_parton_vpar):
    ...
    def __init__(self, num_proc_labels, cfg_drop_prob=0.0, aux_weight=0.1, **kwargs):
        ...
        # Build the auxiliary classifier sharing proc_token Dense layers from self.head.
        # Must be called after super().__init__() has built self.head.
        self.cls_model        = self._build_cls_model()
        self.loss_aux_tracker = keras.metrics.Mean(name="aux")

    # ── Auxiliary classifier sharing proc_token Dense layers ─────────────────

    def _build_cls_model(self):
        """Build a small model that shares proc_emb_1 / proc_emb_2 from self.head
        and adds a softmax classifier on top.  Gradients flow back through the
        shared Dense layers into self.head's cross-attention proc_token path."""
        d1  = self.head.get_layer('proc_emb_1')
        d2  = self.head.get_layer('proc_emb_2')
        P, PF = self.max_partons, self.parton_feat
        inp     = Input(shape=(self.num_cond,), name='cls_cond_in')
        proc_in = inp[:, P * PF + P:]
        x       = d1(proc_in)
        x       = d2(x)
        logits  = layers.Dense(self._num_proc_labels, name='proc_classifier')(x)
        return keras.Model(inputs=inp, outputs=logits, name='proc_cls_head')
    ...
    def _build_vpar_generator_head(self):
        ...
        proc_token = layers.Dense(D, activation='gelu', name='proc_emb_1')(proc_label_in)
        proc_token = layers.Dense(D, name='proc_emb_2')(proc_token)      # (N, D)
...
class WeightedProcLabelPET(ProcLabelPET):
    ...
    def train_step(self, inputs):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # Auxiliary classification loss on the proc_token embedding.
            # Always uses the true label (no dropout — this is not a CFG experiment).
            P, PF           = self.max_partons, self.parton_feat
            proc_label_true = y[:, P * PF + P:]   # one-hot (N, n_proc)
            proc_logits     = self.cls_model(y, training=True)
            loss_aux        = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(
                    labels=proc_label_true, logits=proc_logits))

            loss = loss_jet + loss_part + self.aux_weight * loss_aux

        self.body_optimizer.minimize(loss_part, self.body.trainable_variables, tape=tape)
        # proc_classifier weights are only in cls_model; proc_emb_1/2 are shared
        # with self.head (already in head.trainable_variables) so we add only
        # the classifier head's unique weights to avoid double application.
        cls_extra_vars = self.cls_model.get_layer('proc_classifier').trainable_variables
