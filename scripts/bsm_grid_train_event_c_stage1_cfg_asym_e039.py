"""BSM grid training — E039: joint mass-hypothesis + cone-mass CFG dropout.

Copied from bsm_grid_train_event_c_stage1_cfg_asym.py (E035/E036/E038) and modified
for E039: only the model import changed, from PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym
to PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym_e039 (joint mass_x/mass_y + cone_mass_X/Y
CFG dropout — see that file's docstring for the mechanism). Everything else (argparse,
data loading, stats, optimizer setup) is unchanged; cfg_drop_x_prob/cfg_drop_y_prob still
default to 0.10 each but should be set to 0.35/0.35 at submit time to match E038's
production value (same reasoning: raising exposure to the dropped state trains v_null
better, and the joint-dropout null branch is harder to learn — see obstacle 1 in the
architecture file's docstring). Run name defaults to bsm_grid_event_c_stage1_cfg_asym_e039
(distinct checkpoint dir).

Original E035 docstring follows, for the training loop this file inherits unchanged:

---

Copied from bsm_grid_train_event_c_stage1_cfg.py (E034) and modified for E035:
  - Architecture identical to E034 (stage-1 ResNet, 8-dim combined target).
  - Training change: independent per-cone dropout on cone mass features.
    cone_mass_X (index 4 of 7-dim event vector) dropped with prob cfg_drop_x_prob (default 0.10).
    cone_mass_Y (index 6 of 7-dim event vector) dropped with prob cfg_drop_y_prob (default 0.10).
    Dropout is independent: each event independently draws a Bernoulli mask for X and Y.
  - Tracks four dropout metrics: drop_X (marginal), drop_Y (marginal), drop_both, drop_none.
    Expected with p_x=p_y=0.10: drop_X≈0.10, drop_Y≈0.10, drop_both≈0.01, drop_none≈0.81.
  - Run name: bsm_grid_event_c_stage1_cfg_asym  (distinct checkpoint directory from E034)
  - Stats file: normalisation_stats_event_c_stage1_cfg_asym.json
    (same format as E034; reuse E034 stats with --stats_path if they exist)

Do NOT modify the original bsm_grid_train_event_c_stage1_cfg.py (E034 canonical).

Changelog:
  E034 (2026-08-01): symmetric CFG dropout (both cone masses together at prob=0.15).
  E035 (2026-08-14): asymmetric per-cone CFG dropout (independent, each at prob=0.10).
"""
...
from PET_pp_parton_vpar_bsm_event_c_stage1_cfg_asym_e039 import WeightedBSMPET_event_c_cfg_asym
...
NUM_FEAT        = 6
NUM_JET         = 8   # [log_npart, log1p_MET, sin_MET_phi, cos_MET_phi,
                      #  log1p_cone_pT_X, log1p_cone_mass_X,
                      #  log1p_cone_pT_Y, log1p_cone_mass_Y]
MAX_PARTONS     = 4
PARTON_FEAT     = 7
MASS_NORM       = 600.0
NUM_EVENT_FEAT  = 7   # MET(3) + cone_X(2) + cone_Y(2)
R_CONE          = 1.0
...
# ── Combined stats computation (unchanged from E034) ─────────────────────────
...
def compute_combined_stats_stage1(grid_dir, num_part, val_start, n_signal_files):
    ...
    for fpath in all_files:
        ...
        if not is_bg:
            ...
            ev7      = _assemble_event_feat(raw7).astype(np.float64)

            combined = np.concatenate([log_n[:, None], ev7], axis=1)
...
def load_stats(stats_path):
    ...
    if len(jet_mean) != NUM_JET:
        raise ValueError(
            f"Stats file has {len(jet_mean)}-dim jet_mean, expected {NUM_JET}.")
    return stats


# ── Data loading (unchanged from E034) ───────────────────────────────────────
...
def load_bsm_shard(grid_dir, stats, hvd_rank, hvd_size,
    ...
    jet_mean_s  = float(jet_mean_a[0])
    jet_std_s   = float(jet_std_a[0])
    ...
    jet_all   = np.empty((total_n, NUM_JET),               dtype=np.float32)
    ...
    for fpath, r0, r1, mass_x, mass_y in file_slices:
        ...
        jet_s0    = (log_npart - jet_mean_s) / jet_std_s
        ...
        if is_background:
            ...
            ev_norm = (ev_raw - ev_mean) / ev_std

        jet_combined = np.concatenate([jet_s0, ev_norm], axis=1)
...
def parse_args():
    ...
    p.add_argument('--run_name',           default='bsm_grid_event_c_stage1_cfg_asym_e039')
    p.add_argument('--stats_path',         default=None,
                   help='Path to combined 8-dim stats JSON '
                        '(default: {ckpt_dir}/normalisation_stats_event_c_stage1_cfg_asym.json). '
                        'Pass the E034 stats path to reuse identical stats.')
    p.add_argument('--cfg_drop_x_prob',    type=float, default=0.10,
                   help='Dropout probability for cone_mass_X (index 4 of event vector, default 0.10)')
    p.add_argument('--cfg_drop_y_prob',    type=float, default=0.10,
                   help='Dropout probability for cone_mass_Y (index 6 of event vector, default 0.10)')
...
def main():
    ...
    # ── Compute combined 8-dim stats (rank 0) ────────────────────────────────
    if hvd.rank() == 0 and not os.path.exists(stats_path):
        print("Computing combined E035 asym stats ...", flush=True)
    ...
    num_cond = MAX_PARTONS * PARTON_FEAT + MAX_PARTONS
    if hvd.rank() == 0:
        print(f"num_jet={NUM_JET}  num_event_feat={NUM_EVENT_FEAT}  run={flags.run_name}  "
              f"cfg_drop_x_prob={flags.cfg_drop_x_prob}  cfg_drop_y_prob={flags.cfg_drop_y_prob}",
              flush=True)
    ...
    # ── Build model ───────────────────────────────────────────────────────────
    model = WeightedBSMPET_event_c_cfg_asym(
        num_feat=NUM_FEAT,
        num_jet=NUM_JET,
        ...
        cfg_drop_x_prob=flags.cfg_drop_x_prob,
        cfg_drop_y_prob=flags.cfg_drop_y_prob,
    )
