...
#SBATCH --job-name=e020a_bsm_event
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020a_bsm_event.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020a_bsm_event.err

# E020a — BSM grid training with event-level MET conditioning (3 features)
# Identical setup to E008 (bsm_grid) but with event token in cross-attention KV.
# Event features: log1p(MET_mag), sin(MET_phi), cos(MET_phi)
# Self-resubmitting until training_state.json marks done=true.

SCRIPT=/global/u2/l/lcondren/ContinuousParamFit/omnilearn_pp/scripts/bsm_grid_train_event_a.py
