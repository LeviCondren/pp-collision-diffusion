...
#SBATCH --job-name=auxcls_5proc
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/full_event_mixed/checkpoints/logs/%j_auxcls_5proc.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/full_event_mixed/checkpoints/logs/%j_auxcls_5proc.err

# E006 — auxcls_5proc
# Auxiliary cross-entropy classification loss on the proc_token embedding.
# aux_weight=0.1 (hardcoded default in proc_label_train_auxcls.py; also the CLI default).
# No CFG dropout (cfg_drop_prob=0.0 default in this script).
# Hyperparameters matched to E000 (proc_label_5proc_p3) for apples-to-apples comparison.
# Fresh run — RUN_NAME is new, no existing checkpoint to resume from.
# Self-resubmitting until training_state.json marks done=true.

SCRIPT=/global/u2/l/lcondren/ContinuousParamFit/omnilearn_pp/scripts/proc_label_train_auxcls.py
DATA_DIR=/pscratch/sd/l/lcondren/MCsim/full_event_mixed
RUN_NAME=auxcls_5proc
