#!/bin/bash
# Smoke test for E022 — 1 epoch, tiny data, no self-resubmit.
...
#SBATCH --job-name=smoke_e022
#SBATCH --output=/data/homezvol0/lcondren/pp-collision-diffusion/logs/%j_smoke_e022.out
#SBATCH --error=/data/homezvol0/lcondren/pp-collision-diffusion/logs/%j_smoke_e022.err
...
mkdir -p "${CKPT_DIR}/bsm_grid_event_c_layers4"

echo "Smoke E022 started: $(date)"
python3 -u "${REPO_DIR}/scripts/bsm_grid_train_event_c_layers4.py" \
    --grid_dir           "${GRID_DIR}" \
    --ckpt_dir           "${CKPT_DIR}" \
    --run_name           bsm_grid_event_c_layers4 \
    --val_start          250 \
    --n_train            64 \
    --n_val              32 \
    --batch              16 \
