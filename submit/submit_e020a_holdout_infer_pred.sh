...
#SBATCH --job-name=e020a_infer_pred
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020a_infer_pred.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020a_infer_pred.err

# E020a holdout inference — PREDICTED-FEATURES variant (two-pass).
# Pass 1: E008 baseline (100 steps) → initial particles → compute MET features.
# Pass 2: E020a (500 steps) conditioned on those predicted MET features.
# 4 holdout points in parallel on 4 GPUs: (250,250) (250,300) (300,250) (300,300)
# 5000 events each.

SCRIPT=/global/u2/l/lcondren/ContinuousParamFit/omnilearn_pp/scripts/infer_bsm_grid_event_a_pred.py
GRID_DIR=/pscratch/sd/l/lcondren/MCsim/wprime_signal
RUN_NAME=bsm_grid_event_a
...
echo "E020a checkpoint: ${CKPT_DIR}/${RUN_NAME}/pet_pp.weights.h5"
