...
#SBATCH --job-name=e020c_infer_pred
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020c_infer_pred.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020c_infer_pred.err

# E020c holdout inference — PREDICTED-FEATURES variant (two-pass).
# Pass 1: E008 baseline (100 steps) → initial particles → compute all-7 event features.
# Pass 2: E020c (500 steps) conditioned on those predicted all-7 features.
# 4 holdout points in parallel on 4 GPUs: (250,250) (250,300) (300,250) (300,300)
# 5000 events each.

SCRIPT=/global/u2/l/lcondren/ContinuousParamFit/omnilearn_pp/scripts/infer_bsm_grid_event_c_pred.py
...
echo "E020c checkpoint: ${CKPT_DIR}/${RUN_NAME}/pet_pp.weights.h5"
