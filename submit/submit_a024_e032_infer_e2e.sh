...
#SBATCH --job-name=a024_e032_e2e
#SBATCH --output=/pub/lcondren/wprime_signal_mpi/logs/%j_a024_e032_e2e.out
#SBATCH --error=/pub/lcondren/wprime_signal_mpi/logs/%j_a024_e032_e2e.err

# A024 — E032 (stage-1, num_gen_layers=2, num_jet=8) end-to-end inference on 4 BSM holdout points.
# Uses epoch-127 snapshot to avoid race with ongoing training.
# Stage-1 generates [log_npart, MET, cone_pT/mass_X, cone_pT/mass_Y] jointly from conditioning.
# Stage-2 uses stage-1 output to generate particles.
