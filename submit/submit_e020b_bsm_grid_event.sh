...
#SBATCH --job-name=e020b_bsm_event
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020b_bsm_event.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/wprime_signal/logs/%j_e020b_bsm_event.err

# E020b — BSM grid training with event-level cone_X conditioning (2 features)
# Identical setup to E008 (bsm_grid) but with event token in cross-attention KV.
# Event features: log1p(cone_pT_X), log1p(cone_mass_X)
# Self-resubmitting until training_state.json marks done=true.

SCRIPT=/global/u2/l/lcondren/ContinuousParamFit/omnilearn_pp/scripts/bsm_grid_train_event_b.py
