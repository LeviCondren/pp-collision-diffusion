...
#SBATCH --array=0-3
#SBATCH --job-name=a035_elbo_composable
#SBATCH --output=/pscratch/sd/l/lcondren/MCsim/wprime_signal_mpi/logs/%A_%a_a035_elbo_composable.out
#SBATCH --error=/pscratch/sd/l/lcondren/MCsim/wprime_signal_mpi/logs/%A_%a_a035_elbo_composable.err

# A035 — ELBO posterior inference on A033's composable-CFG generated events
# (E038 checkpoint, epoch 43/200 snapshot, gs_X=gs_Y=1.5). Scores 100 generated
# events per holdout mass point against all 144 grid hypotheses (100 MC
# timestep samples/hypothesis, matching A028's precedent). Output compatible
# with plot_a022_mass_posterior.py.
...
NPZ_DIR=${CKPT_DIR}/${RUN_NAME}/infer_composable_gs1p5
OUT_DIR=${CKPT_DIR}/${RUN_NAME}/mass_inference_a035_composable_gs1p5
